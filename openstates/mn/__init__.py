...
metadata = {
    ...
    'session_details': {
        ...
        '2017s1': {
            ...
            'site_id': '1902017',
            'votes_url': 'http://www.house.leg.state.mn.us/votes/getVotesls9020171.asp',
        },
    },
...
def session_list():
    import requests
    s = requests.Session()
    doc = lxml.html.fromstring(s.get('https://www.revisor.mn.gov/revisor/pages/search_status/status_search.php?body=House', verify=False).text)
    return doc.xpath('//select[@name="session"]/option/text()')
